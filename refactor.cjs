...
// ProductModal badge
code = code.replace(
  /className={`rounded-full px-4 py-2 font-black \$\{product.unavailable \? "bg-red-50 text-red-600" : "bg-emerald-50 text-emerald-700"\}`}/g,
  'className={`rounded-full px-4 py-2 font-black ${product.state === "unavailable" ? "bg-red-50 text-red-600" : product.state === "special_offer" ? "bg-amber-50 text-amber-600" : "bg-emerald-50 text-emerald-700"}`}'
);
code = code.replace(
